...
import { createUserType } from "@/app/services/userType";
...
export default function AddUserType({ onUserTypeAdded }) {
    ...
    const onSubmit = handleSubmit( async (data) => {
        try {
            setLoading(true);
            const response = await createUserType(data); // Call the service
            if (response && response.success) {
                showToast("Tipo de usuario agregado exitosamente", "success", 5000);
                reset(); // Reset the form fields
                if (onUserTypeAdded) {
                    onUserTypeAdded(); //Callback
                }
            } else {
                showToast(response.message || "No se pudo agregar el tipo de usuario", "error", 5000);
            }
        } catch (error) {
            showToast(error.message || "Error al agregar tipo de usuario", "error", 5000);
        } finally {
            setLoading(false);
        }
    })
        ...
        <div className={styles.bodyContainer}>
            <div className={styles.centerContainer}>
                <div className={styles.titleContainer}>
                    <h3 className={styles.titles}>Agregar tipo de usuario</h3>
