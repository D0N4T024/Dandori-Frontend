...
import { updateSupermarket } from "@/app/services/supermarket";
...
export default function UpdateSupermarket({ onSupermarketUpdated, row }) {
    const [loading, setLoading] = useState(false);
    const [checked, setChecked] = useState(row?.isActive || false);
    ...
    const onSubmit = handleSubmit( async (data) => {
        try {
            setLoading(true);
            const response = await updateSupermarket(row.id, data);
            if (response && response.success) {
                showToast("Supermercado editado exitosamente", "success", 5000);
                reset();
                if (onSupermarketUpdated) {
                    onSupermarketUpdated() //Callback
                }
            } else {
                showToast(response.message || "No se pudo editar el supermercado", "error", 5000);
            }
        } catch (error) {
            showToast(error.message || "Error al editar supermercado", "error", 5000);
        } finally {
            setLoading(false);
        }
    })
        ...
        <div className={styles.bodyContainer}>
            <div className={styles.centerContainer}>
                <div className={styles.titleContainer}>
                    <h3 className={styles.titles}>Editar tienda</h3>
                ...
                <form onSubmit={onSubmit} className={styles.fieldsContainer}>
                    ...
                    <div className={styles.field}> {/*Website field*/}
                        <label htmlFor="isActive"><b>Esta activo?</b></label>
                        <Switch
                            {...register("isActive")}
                            checked={checked}
