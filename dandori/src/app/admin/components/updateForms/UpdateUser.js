...
export default function UpdateUser({ onUserUpdated, row }) {
    ...
    const onSubmit = handleSubmit( async (data) => {
        try {
            setLoading(true);
            const response = await updateUser(row.id, data);
            if (response && response.success) {
                showToast("Usuario editado exitosamente", "success", 5000);
                reset();
                if (onUserUpdated) {
                    onUserUpdated() //Callback
                }
            } else {
                showToast(response.message || "No se pudo editar el Usuario", "error", 5000);
            }
        } catch (error) {
            showToast(error.message || "Error al editar Usuario", "error", 5000);
        } finally {
            setLoading(false);
        }
    })
        ...
        <div className={styles.bodyContainer}>
            <div className={styles.centerContainer}>
                <div className={styles.titleContainer}>
                    <h3 className={styles.titles}>Editar usuario</h3>
                ...
                <div className={styles.field}> {/*Name fields*/}
                    ...
                    <div className={styles.field}>
                        ...
                        <Select
                            ...
                            fullWidth
                            defaultValue="678439bae399383c1253fbf9"
